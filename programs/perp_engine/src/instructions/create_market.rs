...
impl MarketParams {
    ...
    pub fn validate(&self) -> Result<u16> {
        ...
        require!(
            ...
            PerpError::InvalidFundingInterval
        );
        require!(
            self.funding_sensitivity_bps <= MAX_FUNDING_SENSITIVITY_BPS,
            PerpError::InvalidFundingSensitivity
        );
        require!(
            self.taker_fee_bps <= MAX_FEE_BPS,
            PerpError::InvalidFeeParam
        );
        require!(
            self.liquidation_penalty_bps <= MAX_LIQUIDATION_PENALTY_BPS,
            PerpError::InvalidPenaltyParam
        );
        require!(self.max_open_interest > 0, PerpError::InvalidMarginParam);
        require!(
        ...
        require!(initial <= BPS_SCALE, PerpError::InvalidMarginParam);
...
pub fn handler(ctx: Context<CreateMarket>, params: MarketParams) -> Result<()> {
    require!(!ctx.accounts.config.paused, PerpError::ProtocolPaused);
