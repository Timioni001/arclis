...
pub fn exposure(
    ...
    mark_price: u64,
) -> Result<TreasuryExposure, PerpError> {
...
pub fn target_delta(stock_qty: u64, hedge_ratio_bps: u16) -> Result<i64, PerpError> {
    ...
    i64::try_from(target).map_err(|_| PerpError::MathOverflow)
}
...
pub fn rebalance_size_delta(
    ...
    i64::try_from(drift.checked_neg().ok_or(PerpError::MathOverflow)?)
        .map_err(|_| PerpError::MathOverflow)
}
...
/// fail.
pub fn nav_per_token(nav: i128, tokens_outstanding: u64) -> Result<u64, PerpError> {
    if tokens_outstanding == 0 || nav <= 0 {
        return Ok(0);
    }
    let per = mul_div(nav, QUOTE_SCALE, i128::from(tokens_outstanding))?;
    u64::try_from(per).map_err(|_| PerpError::MathOverflow)
}
