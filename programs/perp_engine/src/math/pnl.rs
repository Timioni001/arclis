...
#[inline]
pub fn notional(size: i64, price: u64) -> Result<i128, PerpError> {
...
#[inline]
pub fn unrealized_pnl(size: i64, entry_price: u64, mark_price: u64) -> Result<i128, PerpError> {
...
#[inline]
pub fn margin_ratio_bps(equity: i128, notional: i128) -> Result<i128, PerpError> {
    if notional == 0 {
        return Ok(i128::MAX);
    }
...
pub fn weighted_entry_price(
    ...
    let avg = old_leg
        ...
        .checked_div(new_abs)
        .ok_or(PerpError::MathOverflow)?;
    u64::try_from(avg).map_err(|_| PerpError::MathOverflow)
}

/// Fee on a notional amount, rounded up so the protocol never under-charges.
#[inline]
pub fn fee_on_notional(notional: i128, fee_bps: u16) -> Result<i128, PerpError> {
    if fee_bps == 0 {
        return Ok(0);
    }
    let num = notional
