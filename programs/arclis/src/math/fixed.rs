...
#[inline]
pub fn mul_div(a: i128, b: i128, denom: i128) -> Result<i128, ArclisError> {
    if denom == 0 {
...
#[inline]
pub fn mul_div_floor(a: i128, b: i128, denom: i128) -> Result<i128, ArclisError> {
    if denom == 0 {
        return Err(ArclisError::DivideByZero);
    }
    let num = a.checked_mul(b).ok_or(ArclisError::MathOverflow)?;
    let q = num.checked_div(denom).ok_or(ArclisError::MathOverflow)?;
    let r = num.checked_rem(denom).ok_or(ArclisError::MathOverflow)?;
    // Rust truncates toward zero; correct by one when the true quotient is
    // negative and the division was not exact.
    if r != 0 && ((r < 0) != (denom < 0)) {
        q.checked_sub(1).ok_or(ArclisError::MathOverflow)
    } else {
        Ok(q)
    }
}
...
/// phantom credit.
#[inline]
pub fn to_u64(v: i128) -> Result<u64, ArclisError> {
    if v < 0 {
        return Err(ArclisError::NegativeAmount);
    }
    u64::try_from(v).map_err(|_| ArclisError::MathOverflow)
}

/// Narrow an `i128` to `i64`, refusing anything that would wrap.
#[inline]
pub fn to_i64(v: i128) -> Result<i64, ArclisError> {
    i64::try_from(v).map_err(|_| ArclisError::MathOverflow)
}
...
mod tests {
    ...
    fn mul_div_rejects_zero_denominator() {
        assert_eq!(mul_div(1, 1, 0), Err(ArclisError::DivideByZero));
    }

    #[test]
    fn mul_div_overflow_is_caught_not_wrapped() {
        assert_eq!(mul_div(i128::MAX, 2, 1), Err(ArclisError::MathOverflow));
    }
    ...
    fn to_u64_rejects_negatives_instead_of_wrapping() {
        assert_eq!(to_u64(-1), Err(ArclisError::NegativeAmount));
        ...
        assert_eq!(to_u64(u64::MAX as i128 + 1), Err(ArclisError::MathOverflow));
    }

    #[test]
    fn to_i64_bounds() {
        assert_eq!(to_i64(i64::MIN as i128).unwrap(), i64::MIN);
        assert_eq!(to_i64(i64::MAX as i128 + 1), Err(ArclisError::MathOverflow));
    }
}
