...
pub fn settle(
    ...
    notional: i128,
    penalty_bps: u16,
) -> Result<LiquidationOutcome, ArclisError> {
    ...
    if equity <= 0 {
        let deficit = equity.checked_neg().ok_or(ArclisError::MathOverflow)?;
        return Ok(LiquidationOutcome {
            liquidator_reward: 0,
            insurance_cut: 0,
            trader_remainder: 0,
            bad_debt: u64::try_from(deficit).map_err(|_| ArclisError::MathOverflow)?,
        });
    }
    ...
    let trader_remainder = equity
        .checked_sub(penalty)
        .ok_or(ArclisError::MathOverflow)?;

    Ok(LiquidationOutcome {
        liquidator_reward: u64::try_from(liquidator_reward)
            .map_err(|_| ArclisError::MathOverflow)?,
        insurance_cut: u64::try_from(insurance_cut).map_err(|_| ArclisError::MathOverflow)?,
        trader_remainder: u64::try_from(trader_remainder).map_err(|_| ArclisError::MathOverflow)?,
        bad_debt: 0,
    })
}
